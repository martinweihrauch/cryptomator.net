...
namespace CryptomatorLib.Tests.Common
{
    ...
    public class ScryptTest
    {
        ...
        public void TestEmptyString()
        {
            ...
            byte[] key = CryptomatorLib.Common.Scrypt.ScryptDeriveBytes("", Encoding.ASCII.GetBytes(""), 16, 1, 64);
        ...
        public void TestPleaseLetMeInString()
        {
            ...
            byte[] key = CryptomatorLib.Common.Scrypt.ScryptDeriveBytes("pleaseletmein", Encoding.ASCII.GetBytes("SodiumChloride"), 16384, 8, 64);
        ...
        public void TestInvalidParameters()
        {
            ...
            Assert.ThrowsException<ArgumentException>(() =>
                CryptomatorLib.Common.Scrypt.ScryptDeriveBytes("test", Encoding.ASCII.GetBytes("salt"), 15, 1, 64));

            // Test with too small cost parameter
            Assert.ThrowsException<ArgumentException>(() =>
                CryptomatorLib.Common.Scrypt.ScryptDeriveBytes("test", Encoding.ASCII.GetBytes("salt"), 0, 1, 64));

            // Test with too small block size
            Assert.ThrowsException<ArgumentException>(() =>
                CryptomatorLib.Common.Scrypt.ScryptDeriveBytes("test", Encoding.ASCII.GetBytes("salt"), 16, 0, 64));

            // Test with invalid output length
            Assert.ThrowsException<ArgumentException>(() =>
                CryptomatorLib.Common.Scrypt.ScryptDeriveBytes("test", Encoding.ASCII.GetBytes("salt"), 16, 1, 0));
        }
    }
}